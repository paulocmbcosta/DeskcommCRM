...
| `atendimento-protocolo.architecture.json` | protocolo por atendimento e linha do tempo da conversa (migration 0266) — 17 peças, 22 arestas, 4 faixas; a conversa é o fio do canal e o atendimento é o episódio com número próprio; UM trigger em `conversations` alimenta `atendimentos` e `conversation_events`, para que nenhum dos cinco caminhos que mudam o estado da conversa fique sem rastro |
| `pausa-e-limite-por-time.architecture.json` | pausa do atendente com motivo, teto de conversas por time e a fila que acorda (migration 0267) — 17 peças, 23 arestas, 4 faixas; a pausa é indisponibilidade com explicação (tudo que lia `is_available` continua certo), o teto do time é relido DENTRO do claim, e abrir vaga só adianta quem já esperava — sem criar evento para quem não esperava |
| `conectores.architecture.json` | conectores de sistema externo, ligados por organização — o primeiro é o IXC, ERP de provedor (migration 0271) — 25 peças, 37 arestas, 5 faixas; o núcleo não nomeia o conector (só `registro.ts` e `PainelDoConector.tsx` o importam), a identidade é referenciada (só o id do ERP) e o estado é lido ao vivo por uma lista branca que descarta as senhas que o ERP devolve; falha é por seção, e a conexão que falha carimba `erro` para o admin ver em Configurações |
| `chat-do-site.architecture.json` | chat do site, o primeiro canal que não é WhatsApp (migration 0272) — 29 peças, 40 arestas, 6 faixas; o transporte somos nós (a resposta do atendente É a linha de `messages`, e ler é o que promove `sent` → `delivered`), provider (`site_widget`) só em `lib/channels` e MEIO (`site_chat`) para a feature, visitante novo é sempre contato novo, e a única restrição física do canal é não falar primeiro |
| `card-pelo-classificador.architecture.json` | o card nasce quando a conversa é comercial (plano 2026-09-22) — 14 peças, 17 arestas, 6 faixas; o ingest recua no modo `classificador`, o worker (rodando POR ÚLTIMO entre os consumidores de `message.received`) só chama o Jev (TypeSafe, via OpenRouter) para contato **sem** card, lendo só as últimas 12 falas do ATENDIMENTO ATUAL, e o card nasce pela mesma `garantirLeadDaConversa`, com a razão na linha do tempo; falha do classificador cria o card, e falha ao ESCREVER o card LANÇA (nunca some em silêncio); o TURNO DO AGENTE também abre o card — se ele qualifica, negocia ou fecha um contato sem card antes do classificador decidir, o card nasce ali (`mirrorLeadStageToCrm`, decisão do dono 22/09/2026), e esse caminho falha FECHADO (não lança) |
