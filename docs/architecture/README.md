...
| `card-pelo-classificador.architecture.json` | o card nasce quando a conversa é comercial (plano 2026-09-22) — 12 peças, 14 arestas, 5 faixas; o ingest recua no modo `classificador`, o worker (rodando POR ÚLTIMO entre os consumidores de `message.received`) só chama o Jev (TypeSafe, via OpenRouter) para contato **sem** card, lendo só as últimas 12 falas do ATENDIMENTO ATUAL, e o card nasce pela mesma `garantirLeadDaConversa`, com a razão na linha do tempo; falha do classificador cria o card, e falha ao ESCREVER o card LANÇA (nunca some em silêncio) |
