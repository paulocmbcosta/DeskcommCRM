...
| **Conectores de sistema externo — IXC** (`docs/superpowers/specs/2026-09-19-conector-ixc-design.md`, migration 0271) | Fases 1–3 ✅ em 2026-09-19: conexão por organização, identificação por telefone/CPF, aba IXC no painel da conversa (bloqueio, contrato, faturas, OS, conexão, sinal) e enviar a cobrança — desde 2026-09-21 como **boleto em PDF baixado do IXC** (`get_boleto`) ou **Pix com QR code + copia e cola** (`get_pix`), à escolha do atendente, por uma função única (`lib/conectores/ixc/enviar-cobranca.ts`) que a ferramenta da IA vai reusar. Provado pela tela com IXC falso (J26) e, só em leitura agregada, contra um IXC real | **Fase 4 — IA**: bloco de contexto na abertura do atendimento + tools (o engine não tem registro de "context providers"; é a peça nova). **Fase 5**: índice da base para iniciar conversa e campanhas. **Fase 6 — escrita no ERP** (desbloqueio de confiança, abrir OS): exige token de escrita dedicado, e o IXC não tem sandbox. **Não medido**: a tela contra o IXC real, latência a partir da VPS, IXC on-premise. Para ver se uma instalação tem o conector ligado: `select conector, status, verificada_em from conector_conexoes;` |
