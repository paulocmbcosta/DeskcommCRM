...
| U8 `[P0]` | Código **e** `.env` na tag nova, digest em dia, mas os CONTÊINERES na anterior: o `update.sh` atualiza | coberto — `tests/shell/update-guard.test.sh` caso 13. É a lacuna que sobrou depois do caso 12, na mesma família: o `update.sh` regrava o `.env` **antes** do `docker pull` e do `up -d`, então uma interrupção entre o fim do pull e o `up -d` deixa código, `.env` e imagem na versão nova e os três contêineres na anterior — e aí o 1º critério vê o `.env` no alvo, o 2º vê digest local == remoto, e a resposta é "Nada a atualizar". O 3º critério lê `{{.Config.Image}}` dos contêineres, que é a única sonda que olha para o que EXECUTA. Carrega controle (13b: contêineres já no alvo → "Nada a atualizar") porque o dublê de `docker` responde vazio a `docker inspect` e a prova passaria por vacuidade, mais as prudências que o custo de falso positivo exige: stack parada não acusa (13c), canal móvel segue decidido pelo digest (13d), `docker` fora do ar não derruba o script (13g). Cinco sabotagens, uma por regra, todas reprovando. **NÃO medido:** numa VPS real — a lacuna foi lida no código, nunca reproduzida em máquina, e a prova é com `docker` dublado. Também **não medido em tela**: num estado "código na tag, contêiner na anterior" nascido FORA de um run registrado, o rodapé de versão nomeia a versão do código (o `agent.sh` manda `git describe` do host), e `app/api/v1/system/version/route.ts` só compensa isso para rollback gravado em `system_update_runs` |
